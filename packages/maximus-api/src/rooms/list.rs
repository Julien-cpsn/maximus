...
use crate::user::avatar::fetch_user_avatar;
...
pub async fn get_user_rooms() -> Result<UserRoomList> {
    ...
    for matrix_room in matrix_rooms {
            ...
            .filter_map(|e| async move {
                if let Ok(parent_space) = e {
                    match parent_space {
                        ...
                        | ParentSpace::Illegitimate(room) => Some(room.room_id().to_string()),
        ...
        let room_type = match matrix_room.room_type() {
            None => RoomType::Text,
            Some(room_type) => match room_type {
                ruma::room::RoomType::Space => RoomType::Space,
                ruma::room::RoomType::_Custom(_) => RoomType::Call,
                _ => RoomType::Text
            }
        };

        let mut room = RoomPreview {
            ...
            children: vec![],
            avatar: None,
            unread_messages_count: matrix_room.unread_notification_counts().notification_count,
        };

        debug!("{:#?}", room);
        debug!("{:?}", matrix_room.unread_notification_counts());

        if matrix_room.is_direct().await? {
            let users = matrix_room.direct_targets();
            let user = users.iter().next().expect("No users in room");
            let dm_user_id = user.as_user_id().unwrap();

            room.avatar = fetch_user_avatar(&client, dm_user_id.to_owned()).await?;

            dms.insert(room_id, room);
        }
        else {
            room.avatar = avatar.map(|data| general_purpose::STANDARD.encode(&data));
